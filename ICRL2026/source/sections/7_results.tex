...
Due to the high cost and resource requirements of human evaluations, this work reuses the subjective evaluation results from \textbf{DiffuseStyleGesture}~\cite{yang2023diffusestylegesture} as a baseline. The proposed \textbf{DeepGesture} model is not included in the subjective study.

To assess visual quality, a separate user study is conducted comparing the proposed method with real motion capture data. Each evaluated clip is 11–51 seconds long (mean: 31.6s), exceeding the typical GENEA clip duration of 8–10s~\cite{yoon2022genea}. Longer clips offer clearer insight into gesture consistency~\cite{yang2022reprgesture}. Ratings range from 5 (\texttt{excellent}) to 1 (\texttt{bad}).
