...
%Similar to previous methods \citep{yoon2022genea}, \citep{kucherenko2021large}, speech-driven gesture generation models still lack objective metrics that consistently reflect human subjective perception \citep{alexanderson2022listen}.
%
...
%
%This work reuses the evaluation results of the baseline model \textbf{DiffuseStyleGesture} \citep{yang2023diffusestylegesture} for human perception metrics, as gesture generation remains a nascent field and the cost of evaluating multiple models is high. Therefore, this paper does not include evaluation results for the proposed \textbf{DeepGesture} model.
%
%%\begin{figure}[htbp]
%%	\centering
%%	\begin{subfigure}[b]{0.3\linewidth}
%%		\includegraphics[width=\linewidth]{figures/BoxHumanLikeness.pdf}
%%		\caption*{\footnotesize (a) Human-likeness}
%%	\end{subfigure}
%%	\hfill
%%	\begin{subfigure}[b]{0.3\linewidth}
%%		\includegraphics[width=\linewidth]{figures/BoxSpeechAppropriateness.pdf}
%%		\caption*{\footnotesize (b) Speech Appropriateness}
%%	\end{subfigure}
%%	\hfill
%%	\begin{subfigure}[b]{0.3\linewidth}
%%		\includegraphics[width=\linewidth]{figures/BoxStyleAppropriateness.pdf}
%%		\caption*{\footnotesize(c) Style Appropriateness}
%%	\end{subfigure}
%%	\label{fig:compare}
%%\end{figure}
%
%To understand the visual performance of the proposed method, this paper conducts a user study comparing gestures generated by the proposed method with those from real motion capture data. The duration of the evaluated video clips ranges from 11 to 51 seconds, with an average length of 31.6 seconds - longer than clips used in the GENEA evaluation \citep{yoon2022genea} (8–10 seconds), as longer durations can provide clearer and more convincing results \citep{yang2022reprgesture}. Participants rated each video on a scale from 5 to 1, labeled $\texttt{excellent}$, $\texttt{good}$, $\texttt{fair}$, $\texttt{poor}$, and $\texttt{bad}$. 
%
%\begin{table}[h]
%	\centering
%	\begin{tabular}{lcc}
...
%			\hline
%			Proposed Model                     &         &         &         &           &          &                 \\
%			\quad \textbf{DeepGesture} & 161.22 & 89.58 & 279.95 & 156.93   & 99.86   & 215.24    \\
%			\hline
%		\end{tabular}%
%	}
...
%%		Ground Truth             & -       & -          \\
%%		Ours                     &       & \\
%%		\quad DeepGesture (Feature D=1141) & 2.058      & 9465.546 \\
%%		\quad DeepGesture (Rotations) & 3.513       & 9519.129 \\
%%		\hline
%%	\end{tabular}
...
%	
%	\item \textbf{Rotations:} From the resulting BVH file, this paper extracts the rotation angles, $D = 225$ ($225 = 75 \times 3$), from the gesture sequence of length $M$ frames for evaluation in the DeepGesture row below.
%\end{itemize}
%
...
%\textbf{Mean Opinion Scores (MOS)}
%
%As of now, there is no standardized metric for gesture generation, particularly in the speech-driven context. Consequently, this work relies on subjective human evaluations, a practice consistent with prior research~\citep{yoon2022genea, kucherenko2021large, alexanderson2022listen}, since objective metrics still fail to fully capture human perception.
%
...
\textbf{MOS Evaluation}

Due to the high cost and resource requirements of human evaluations, this work reuses the subjective evaluation results from \textbf{DiffuseStyleGesture}~\citep{yang2023diffusestylegesture} as a baseline. The proposed \textbf{DeepGesture} model is not included in the subjective study.

To assess visual quality, a separate user study is conducted comparing the proposed method with real motion capture data. Each evaluated clip is 11–51 seconds long (mean: 31.6s), exceeding the typical GENEA clip duration of 8–10s~\citep{yoon2022genea}. Longer clips offer clearer insight into gesture consistency~\citep{yang2022reprgesture}. Ratings range from 5 (\texttt{excellent}) to 1 (\texttt{bad}).
...
\begin{table}[h]
	...
	\begin{tabular}{lcc}
		...
		\hline
	\end{tabular}
	\caption{Mean Opinion Score (MOS) evaluation results}
	\label{table:MOSScore}
\end{table}
...
%This work introduces FGD and implements the open-source tool \hyperlink{https://github.com/GestureScore/GestureScore}{GestureScore}\footnote{\url{https://github.com/GestureScore/GestureScore}}
...
%\subsection{Standardizing Gesture Generation Evaluation}
%
%Gesture generation research lacks consistent evaluation protocols. Existing metrics such as FID and IS are insufficient for capturing the perceptual qualities of gestures. Moreover, the use of different datasets across studies prevents meaningful comparisons.
%
%To address this, we propose a shared benchmark platform: the \hyperlink{https://genea-workshop.github.io/leaderboard/}{GENEA Leaderboard}~\citep{nagy2024towards}. This leaderboard curates and standardizes data across multiple languages, then invites authors to evaluate their models on the same dataset. Human evaluations are performed via Prolific.
%
%We also develop the open-source system \hyperlink{https://github.com/hemvip/hemvip.github.io}{hemvip.github.io}\footnote{HEMVIP2: \url{https://github.com/hemvip/hemvip.github.io}} to streamline Prolific-based evaluations. DeepGesture model evaluations will be added to the leaderboard via this system.
%
%This initiative aims to establish a unified evaluation protocol, promoting reproducibility, comparability, and progress in gesture generation research.
...
%Three speech-gesture datasets are used in this paper: the Trinity dataset \citep{ferstl2018investigating}, the TED dataset \citep{yoon2019robots}, and a Chinese dataset collected for this work.
%
%Trinity Gesture dataset is a large database of speech and gestures jointly collected by \citet{ferstl2018investigating}. This dataset consists of $242$ minutes of motion capture and audio of one male actor talking on different topics. The actor’s motion was captured with a $20$-camera Vicon system and solved onto a skeleton with $69$ joints. In this paper, we use the official release version of \emph{The GENEA Challenge $2020$ Dataset} \citep{kucherenko2021large}, where $221$ minutes are used as training data, and the remaining $21$ minutes are kept for testing.
%
%TED Gesture dataset \citep{yoon2019robots} is a 3D upper-body gesture dataset collected using 3D pose estimation from English TED videos. This dataset includes 3D upper-body gestures of the speakers, the aligned speech audios, and text transcripts. In total, there are $253,186$ data samples, $80\%$ of which are training samples, $10\%$ belong to the validation set, and the remaining $10\%$ are test samples. The duration of each data sample is $34$ frames at a rate of $15$ fps, so the total length of this dataset is about $97$ hours. Notably, we adapt our model to take 3D joint positions instead of rotations for the TED dataset. The generated gestures are also represented in 3D joint positions, which are then converted into joint rotations for visualization.
%
...
%We choose four recent successful 3D co-speech gesture synthesis systems for comparison. On the Trinity dataset, we compare with the systems of Style Gesture (SG) \citep{alexanderson2020style} and Gesticulator (Ges) \citep{kucherenko2020gesticulator}. SG generates gestures based on only the speech audio and uses a normalizing flow model. Ges leverages the audio and text of the speech to generate semantically consistent gestures. On the TED dataset, we compare the systems of Gestures from Trimodal Context (GTC) \citep{yoon2020speech} and Speech to Affective Gestures (S2AG) \citep{bhattacharya2021speech2affectivegestures}. GTC uses speech audio, text transcript, and speaker identity to generate gestures. Based on the three modalities used in GTC, S2AG adds another new modality of affective expressions from the seed poses into their model.
%
...
%We further conduct user studies to assess the performance of our system qualitatively, where SG \citep{alexanderson2020style} and Ges \citep{kucherenko2020gesticulator} are used as the baseline methods. 
...
%Similar to \citet{aristidou2021rhythm}, we calculate the motion beats based on the motion deceleration \citep{davis2018visual}.
